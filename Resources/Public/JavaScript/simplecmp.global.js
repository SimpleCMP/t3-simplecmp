...
`,w=y`
  ${yr}
  ${he}
`;function br(r,e){if(r!==void 0){if(typeof r=="string")return r;if(typeof r=="object")return r[e]??r.default}}var M=class extends b{constructor(){super(...arguments);this.testing=!1;this._handleAccept=()=>{this.manager!==void 0&&(this.manager.changeAll(!0),this.manager.saveAndApplyConsents("accept"),this._emit("accept"))};this._handleDecline=()=>{this.manager!==void 0&&(this.manager.changeAll(!1),this.manager.saveAndApplyConsents("decline"),this._emit("decline"))};this._handleConfigure=t=>{t.preventDefault(),this._emit("configure")}}connectedCallback(){super.connectedCallback(),this.config?.autoFocus===!0&&queueMicrotask(()=>this.focus())}render(){let t=this.config,i=this.manager;if(t===void 0||i===void 0)return p;if(!this.testing&&i.confirmed)return p;if(t.noNotice===!0)return p;let n=this._activeLang(),o=this._resolvePolicyUrl(t.privacyPolicy,["privacyPolicyUrl"],n),s=this._resolvePolicyUrl(t.imprint,["imprintUrl"],n),a=this._t(["!","consentNotice","title"]),c=t.showNoticeTitle===!0&&a!==void 0,l=t.htmlTexts===!0,f=o?d`<a href=${o}>${this._t(["privacyPolicy","name"])}</a>`:"",m=s?d`<a href=${s}>${this._imprintLinkText()}</a>`:"",u=d`<a
      href="#"
      @click=${this._handleConfigure}
      >${this._t(["consentNotice","learnMore"])}</a
    >`,g=this._t(["consentNotice","description"],{purposes:d`<strong>${this._purposesText(t)}</strong>`,privacyPolicy:f,imprint:m,learnMoreLink:u});return d`
      <div
        class="cn-body"
        role="dialog"
        aria-labelledby=${ct(c?"cn-title":void 0)}
        aria-label=${ct(c?void 0:a)}
        aria-describedby="cn-description"
        tabindex="0"
      >
        ${c?d`<h2 id="cn-title">${a}</h2>`:p}
        <p id="cn-description">${l?kr(g):g}</p>
        ${this._renderPolicyLinks(o,s)}
        ${i.changed?d`<p class="cn-changes">${this._t(["consentNotice","changeDescription"])}</p>`:p}
        ${this.testing?d`<p>${this._t(["consentNotice","testing"])}</p>`:p}
        ${this._renderButtonRow(t)}
      </div>
    `}_renderButtonRow(t){let i=this._resolveLayout(t),n=i!=="compact"&&t.hideLearnMore!==!0,o=t.hideDeclineAll!==!0;return d`<div class="cn-buttons cn-layout-${i}">
      ${n?d`<button
            type="button"
            class="cn-configure"
            @click=${this._handleConfigure}
    ...
    </div>`}_resolveLayout(t){let i=(t.layout??"standard").toString().toLowerCase();return i==="compact"||i==="stacked"?i:"standard"}_activeLang(){return this.config?.lang??document.documentElement.lang??"en"}_purposesText(t){let i=t.purposeOrder??[],o=Tt(t).filter(c=>c!=="functional").sort((c,l)=>i.indexOf(c)-i.indexOf(l)).map(c=>this._tString(["!","purposes",c,"title?"])||x(c));if(o.length<=1)return o[0]??"";let s=o.slice(0,-2),a=o.slice(-2).join(" & ");return[...s,a].join(", ")}_resolvePolicyUrl(t,i,n){let o=br(t,n);if(o!==void 0)return o;let s=this._tString(["!",...i]);return s===""?void 0:s}_imprintLinkText(){return this._tString(["!","consentNotice","imprint","name"])||this._tString(["!","imprint","name"])||"Imprint"}_tString(t){let i=this._t(t);return typeof i=="string"?i:Array.isArray(i)?i.map(n=>typeof n=="string"?n:"").join(""):""}_renderPolicyLinks(t,i){return t===void 0&&i===void 0?p:d`
      <p class="cn-policy-links">
        ${t?d`<a href=${t}>${this._t(["privacyPolicy","name"])}</a>`:p}
        ${t&&i?" \xB7 ":p}
        ${i?d`<a href=${i}>${this._imprintLinkText()}</a>`:p}
      </p>
    `}};M.styles=[w,y`
      :host {
        display: block;
    ...
    `}_renderBody(t){let i=[],n=(o,s,a=!1)=>{if(s===void 0||s==="")return;let c=this._tString(["providerInfo","field",o])||o,l=a&&ge(s)?d`<a href=${s} target="_blank" rel="noopener noreferrer">${s}</a>`:d`${s}`;i.push(d`<dt>${c}</dt><dd>${l}</dd>`)};return n("vendor",t.vendor),n("description",t.vendorDescription),n("address",t.vendorAddress),n("country",t.vendorCountry),n("privacyPolicy",t.privacyPolicyUrl,!0),n("optOut",t.vendorOptOutUrl,!0),n("partner",t.vendorPartner),i.length===0?d`<p class="empty">
        ${this._tString(["providerInfo","noData"])||"No provider information available."}
      </p>`:d`<dl>${i}</dl>`}_tString(t){let i=this._t(t);return typeof i=="string"?i:Array.isArray(i)?i.map(n=>typeof n=="string"?n:"").join(""):""}};T.styles=[w,y`
      :host {
        display: contents;
      }
    ...
    `:p}_renderProviderInfoModal(t){return this._providerInfoOpen?d`
    ...
    `],h([v({attribute:!1})],D.prototype,"service",2),h([v({type:String,attribute:"service-name"})],D.prototype,"serviceName",2),h([Te()],D.prototype,"_providerInfoOpen",2),D=h([k("simplecmp-contextual-notice")],D);var U=class extends b{constructor(){super(...arguments);this.visible=!0;this._onChange=t=>{let i=t.target.checked,n=this.service;n===void 0||n.required||(this.manager?.updateConsent(n.name,i),this._emit("service-toggle",{name:n.name,value:i}))}}render(){let t=this.service;if(t===void 0)return p;let i=`simplecmp-service-${t.name}`,n=t.required===!0||this.manager?.consents[t.name]===!0,o=this._tString(["!",t.name,"title?"])||x(t.name),s=this._tString(["!",t.name,"description?"])||void 0;return d`
      <div class="row">
        <input
          type="checkbox"
          id=${i}
        ...
        <div class="meta">
          <label for=${i}>
            ...
            ${t.required?d`<span class="badge">${this._t(["service","required","title"])}</span>`:p}
            ${t.optOut?d`<span class="badge">${this._t(["service","optOut","title"])}</span>`:p}
          </label>
          ${s?d`<p class="description">${s}</p>`:p}
          ${this._renderPurposes(t)}
        </div>
      </div>
    `}_renderPurposes(t){let i=t.purposes??[];if(i.length===0)return p;let n=i.map(s=>this._tString(["!","purposes",s,"title?"])||x(s)).join(", "),o=this._t(["service",i.length>1?"purposes":"purpose"]);return d`<p class="purposes">${o}: ${n}</p>`}_tString(t){let i=this._t(t);return typeof i=="string"?i:Array.isArray(i)?i.map(n=>typeof n=="string"?n:"").join(""):""}};U.styles=[he,y`
      :host {
        display: block;
    ...
    `],h([v({attribute:!1})],U.prototype,"service",2),h([v({type:Boolean})],U.prototype,"visible",2),U=h([k("simplecmp-service-toggle")],U);var O=class extends b{constructor(){super(...arguments);this.purpose="";this.services=[];this._expanded=!1;this._onMasterChange=t=>{let i=t.target.checked;if(this.manager!==void 0){for(let n of this.services)n.required!==!0&&this.manager.updateConsent(n.name,i);this._emit("purpose-toggle",{purpose:this.purpose,value:i})}};this._toggleExpanded=t=>{t.preventDefault(),this._expanded=!this._expanded}}render(){if(this.manager===void 0)return p;let t=this._computeStatus(),i=this._tString(["!","purposes",this.purpose,"title?"])||x(this.purpose),n=this._tString(["!","purposes",this.purpose,"description"]),o=`simplecmp-purpose-${this.purpose}`;return d`
      <div class="header">
        <input
          type="checkbox"
          id=${o}
    ...
    `}_renderPolicyLinks(t,i){let n=t!==void 0&&ge(t)?t:void 0,o=i!==void 0&&ge(i)?i:void 0;return n===void 0&&o===void 0?p:d`
      <p class="policy-links">
        ${n?d`<a href=${n} target="_blank" rel="noopener"
              >${this._t(["privacyPolicy","name"])}</a
            >`:p}
        ${n&&o?" \xB7 ":p}
        ${o?d`<a href=${o} target="_blank" rel="noopener"
              >${this._imprintLinkText()}</a
            >`:p}
      </p>
    `}_activeLang(){return this.config?.lang??document.documentElement.lang??"en"}_collectFocusable(){let t=this._dialog;if(t===void 0)return[];let i=[],n='button:not([disabled]), a[href], input:not([disabled]):not([type="hidden"]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])',o=s=>{if(s.matches(n)&&this._isVisible(s)&&i.push(s),s.shadowRoot!==null)for(let a of s.shadowRoot.children)o(a);for(let a of s.children)o(a)};for(let s of t.children)o(s);return i}_deepActiveElement(){let t=document.activeElement;for(;t?.shadowRoot?.activeElement;)t=t.shadowRoot.activeElement;return t}_isVisible(t){let i=t;return typeof i.checkVisibility=="function"?i.checkVisibility():t.offsetParent!==null}_collectPurposes(){let t=new Map;for(let i of this.config?.services??[])for(let n of i.purposes??[]){let o=t.get(n)??[];o.push(i),t.set(n,o)}return t}_resolvePolicyUrl(t,i,n){let o=wr(t,n);if(o!==void 0)return o;let s=this._tString(["!",...i]);return s===""?void 0:s}_imprintLinkText(){return this._tString(["!","consentNotice","imprint","name"])||this._tString(["!","imprint","name"])||"Imprint"}_tString(t){let i=this._t(t);return typeof i=="string"?i:Array.isArray(i)?i.map(n=>typeof n=="string"?n:"").join(""):""}};S.styles=[w,y`
      :host {
        display: contents;
      }
    ...
    `],h([v({type:Boolean,reflect:!0})],S.prototype,"open",2),h([v({type:Boolean})],S.prototype,"testing",2),h([De("dialog")],S.prototype,"_dialog",2),S=h([k("simplecmp-modal")],S);function Sr(r){return typeof r=="string"?Z(r):Array.isArray(r)?r.map(e=>typeof e=="string"?Z(e):e):r}var C=class extends b{constructor(){super(...arguments);this.position="bottom-right";this._onClick=t=>{t.preventDefault(),this._emit("trigger-click")}}connectedCallback(){super.connectedCallback(),this.setAttribute("position",this.position)}render(){let t=this._resolveLabel();return d`
    ...
    `}_resolveLabel(){if(this.label!==void 0&&this.label!=="")return this.label;if(this.config!==void 0){let t=this._t(["!","floatingTrigger","label"]);if(typeof t=="string"&&t!=="")return t;if(Array.isArray(t)&&t.length>0)return t.map(i=>typeof i=="string"?i:"").join("")}return"Cookie settings"}};C.styles=[w,y`
      :host {
        display: contents;
      }
    ...
    `],h([v({type:String})],C.prototype,"position",2),h([v({type:String})],C.prototype,"label",2),C=h([k("simplecmp-trigger")],C);function Ri(r){let e=ne(r),t=r.domMode==="light",i;r.noNotice!==!0&&(!e.confirmed||r.testing===!0)&&(i=new M,i.config=r,i.manager=e,r.testing===!0&&(i.testing=!0),t&&i.setAttribute("mode","light"),document.body.appendChild(i));let o=new S;o.config=r,o.manager=e,r.testing===!0&&(o.testing=!0),t&&o.setAttribute("mode","light"),document.body.appendChild(o),r.mustConsent===!0&&!e.confirmed&&(o.open=!0);let s=()=>{o.open=!0};document.addEventListener("simplecmp:configure",s);let a={update(f,m){m==="saveConsents"&&i!==void 0&&(i.remove(),i=void 0)}};e.watch(a);let c,l;return r.floatingTrigger&&(c=new C,c.config=r,typeof r.floatingTrigger=="object"&&(r.floatingTrigger.position!==void 0&&(c.position=r.floatingTrigger.position),r.floatingTrigger.label!==void 0&&(c.label=r.floatingTrigger.label)),t&&c.setAttribute("mode","light"),document.body.appendChild(c),l=()=>{o.open=!0},c.addEventListener("simplecmp:trigger-click",l)),e.applyConsents(),{show(){o.open=!0},hide(){o.open=!1},destroy(){e.unwatch(a),document.removeEventListener("simplecmp:configure",s),i?.remove(),o.remove(),c!==void 0&&l!==void 0&&(c.removeEventListener("simplecmp:trigger-click",l),c.remove())},manager:e}}var ji=`:where(
    simplecmp-banner,
    simplecmp-modal,
    simplecmp-purpose-group,
    simplecmp-service-toggle,
...
`;var lt="data-simplecmp-theme",Bi={bootstrap5:ji,tailwind4:Ni,bulma:Li,pico:Ii};function Ui(r){if(typeof document>"u")return;let e=document.querySelector(`style[${lt}]`);if(r===void 0||r==="default"){e?.remove();return}let t=Bi[r];if(t===void 0){console.warn(`SimpleCMP: theme "${r}" has no adapter; ignoring. Known themes: ${Object.keys(Bi).join(", ")}, 'default'.`);return}if(e?.getAttribute(lt)===r)return;let i=e instanceof HTMLStyleElement?e:document.createElement("style");i.setAttribute(lt,r),i.textContent=t,i.isConnected||document.head.appendChild(i)}function _r(r){return yt(r)}function Ar(r){return bt(r)}function xr(r){return Re(r)}z(Ke,F(pi));var Cr="0.0.1",J=null,H=null;function zr(r){Br(r),Ur(r);let e=Ir(),t=e?{...r,testing:!0}:r;e&&Nr(),J!==null&&(J.destroy(),J=null),H!==null&&(H(),H=null),Ui(r.theme);let i=ne(t);t.record&&Tr(t),t.interceptRuntime&&(H=$r(t,i));let n=null,o=[],s=()=>{n=Ri(t);for(let c of o)c==="show"?n.show():n.hide();o.length=0},a=null;return typeof document<"u"&&document.body!==null?s():typeof document<"u"&&(a=s,document.addEventListener("DOMContentLoaded",a,{once:!0})),J={show:()=>{n!==null?n.show():o.push("show")},hide:()=>{n!==null?n.hide():o.push("hide")},manager:i,destroy:()=>{H!==null&&(H(),H=null),a!==null&&typeof document<"u"&&(document.removeEventListener("DOMContentLoaded",a),a=null),n?.destroy(),n=null,o.length=0}},J}function $r(r,e){let t=typeof r.interceptRuntime=="object"&&r.interceptRuntime!==null?r.interceptRuntime:{},i=mi(r.services,{blockAllUnknown:t.universalBlock===!0}),n=t.onBlock;return ui({matcher:i,consentChecker:o=>e.getConsent(o),sameOriginHosts:t.sameOriginHosts,onBlock:o=>{R!==null&&R.recordSyntheticDetection({kind:Er(o.mechanism),identifier:o.url,origin:Pr(o.url)}),n?.(o)}})}function Er(r){switch(r){case"script-src":return"script";case"iframe-src":return"iframe";case"img-src":return"image";case"fetch":case"xhr":case"sendBeacon":return"request"}}function Pr(r){try{return new URL(r,window.location.href).hostname||void 0}catch{return}}function Mr(){J?.show()}var R=null;function Tr(r){R&&(R.stop(),R=null);let e=typeof r.record=="object"&&r.record!==null?{...r.record}:{};if(!e.storageName&&typeof r.storageName=="string"&&(e.storageName=r.storageName),e.storageName){let s=e.ignoreCookies??[];e.ignoreCookies=s.includes(e.storageName)?s:[e.storageName,...s]}let t=r.services??[],i=r.serviceDbUrl?new K(new W({url:r.serviceDbUrl,auth:r.serviceDbAuth}),t):null,n=i??new V(t),o=new _e({options:e,classifier:n,services:t,watcherFactories:[s=>new Ae(s,{intervalMs:e.cookieIntervalMs}),s=>new xe(s),s=>new Ce(s)],onDetectionForLibEvent:s=>{Ge("recorderDetection",s)}});if(i&&i.onEnrichment((s,a)=>{o.enrichDetection(s,a)}),r.cmsBridgeUrl){let s=Lr(),a=new j({url:r.cmsBridgeUrl,auth:r.cmsBridgeAuth,source:r.cmsBridge?.source??e.storageName??"default",dedupTtlMs:r.cmsBridge?.dedupTtlMs,crossSessionDedupMs:s?0:r.cmsBridge?.crossSessionDedupMs,flushDebounceMs:r.cmsBridge?.flushDebounceMs,maxBatchSize:r.cmsBridge?.maxBatchSize,sampleRate:s?1:r.cmsBridge?.sampleRate,respectDoNotTrack:s?!1:r.cmsBridge?.respectDoNotTrack,timeoutMs:r.cmsBridge?.timeoutMs,reportGeneration:r.cmsBridge?.reportGeneration});o.on("detectionSettled",c=>a.onDetection(c))}R=o,R.start()}function Dr(){return R??void 0}var Or=jt,Rr=ne,jr=te;function Lr(){if(typeof window>"u"||typeof URLSearchParams>"u")return!1;try{return new URLSearchParams(window.location.search).get("simplecmp_discover")==="1"}catch{return!1}}function Ir(){if(typeof window>"u"||typeof URLSearchParams>"u")return!1;try{if(new URLSearchParams(window.location.search).get("simplecmp_audit")==="1")return!0;let e=window.location.hash;return e.length>1&&new URLSearchParams(e.slice(1)).get("simplecmp_audit")==="1"}catch{return!1}}function Nr(){typeof window>"u"||requestAnimationFrame(()=>{requestAnimationFrame(()=>{requestAnimationFrame(()=>{let r=Re();try{window.parent.postMessage({type:"simplecmp-audit-from-fe",results:r,location:{href:window.location.href,host:window.location.host}},"*")}catch{}})})})}function Br(r){r.cmsBridgeUrl&&!r.record&&console.warn("SimpleCMP: `cmsBridgeUrl` is set but `record` is not enabled. The CMS bridge listens to recorder detections \u2014 without the recorder running, no webhooks will ever fire. Set `record: true` or remove `cmsBridgeUrl`.")}function Ur(r){r.hideDeclineAll&&console.warn('SimpleCMP: `hideDeclineAll: true` hides the "Decline all" button on the first banner level. This is incompatible with German consent requirements (BGH "Cookie II", BGH I ZR 7/16; DSK 2022). Keep the decline option equally prominent or expect compliance issues.')}return Ki(Hr);})();
